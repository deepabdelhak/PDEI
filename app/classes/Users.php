...
class Users
{
			...
			if ($result != false) {
				...
				if (password_verify($password, $value['password'])) {
					...
					if ($value['status'] == '1') {
						...
						// Add this line to set the city in the session
						Session::set("function", $value['country']);
	
						
						echo "<script>location.href='dec_add.php';</script>";
						exit();
					}
				} else {
	...
	public function createNewUserData($data, $file)
	{
		...
		if ($name == "" || $phone == "" || $email == "" || $city == "" || $country == "" || $password == "" || $confirm_password == "" || $rolename == "" || $status == "" || $create_date == "" || $gendar == "") {
    ...
    <strong>Error !</strong> Please fill up Valid Email !</div>';
	    ...
	    <strong>Error !</strong> You can upload only:-' . implode(', ', $permited) . '</div>';
			...
			if ($mailCheck != false) {
				...
				$has_pass = password_hash($password, PASSWORD_DEFAULT);
				$query = "INSERT INTO $this->table(name,  phone, email,city,country, password,  profilePhoto, rolename,  status, gendar,  create_date) VALUES('$name', '$phone', '$email','$city','$country', '$has_pass', '$uploaded_image','$rolename', '$status', '$gendar', '$create_date')";
				$inserted_rows = $this->db->insert($query);
	...
	public function updateUserById($data, $file, $id)
	{
		...
		//|| $address == ""
		if ($name == "" || $phone == ""  || $email == "" || $city == "" || $country == "" || $rolename == "" || $status == "" || $create_date == "" || $gendar == "") {
			...
			if (!empty($file_name)) {
				...
				if ($updated_row) {
					...
					if ($sendmail) {
						...
						$msg = '<div class="alert alert-danger alert-dismissible" id="flash-msg">
					    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
					    <strong>Error !</strong> User Updated !</div>';
						return $msg;
					}
	...
	public function deleteEventById($eventId)
	{
		...
		$this->db->delete("DELETE FROM eventcauses WHERE event_id = '$eventId'");
		$this->db->delete("DELETE FROM eventconsequences WHERE event_id = '$eventId'");
		$this->db->delete("DELETE FROM eventdetails WHERE event_id = '$eventId'");
		$this->db->delete("DELETE FROM eventotherdetails WHERE event_id = '$eventId'");
	...
	public function selectAllDeclaration($userId)
	{
		$userId = filter_var($userId, FILTER_SANITIZE_NUMBER_INT);

		// First, get the user's role
		$roleQuery = "SELECT rolename FROM tbl_users WHERE userid = '$userId' LIMIT 1";
		$roleResult = $this->db->select($roleQuery);

		if ($roleResult && $roleResult->num_rows > 0) {
			$userRole = $roleResult->fetch_assoc()['rolename'];

			// If user is Author, show all declarations
			if (strtolower($userRole) === 'author') {
				$query = "SELECT d.*, u.name as declarant_name 
						 FROM $this->table_dec d 
						 LEFT JOIN tbl_users u ON d.userid = u.userid 
						 ORDER BY d.event_id DESC";
			}
			// Otherwise, show only user's declarations
			else {
				$query = "SELECT d.*, u.name as declarant_name 
						 FROM $this->table_dec d 
						 LEFT JOIN tbl_users u ON d.userid = u.userid 
	...
	// Helper function to check if user is Author
	public function isAdmin($userId)
	{
		$userId = filter_var($userId, FILTER_SANITIZE_NUMBER_INT);
		$query = "SELECT rolename FROM tbl_users WHERE userid = '$userId' LIMIT 1";
		$result = $this->db->select($query);

		if ($result && $result->num_rows > 0) {
			$userRole = $result->fetch_assoc()['rolename'];
			return strtolower($userRole) === 'author';
		}
	...
	public function getPrincipalTitleForEvent($event_id)
	{
		$query = "SELECT 
		e.event_id,
	...
	// New Users method
	public function SoinsEtUsagers()
	{
		$query = "SELECT DISTINCT e.*
		FROM principal_title pt
		JOIN eventcategories ec ON pt.id = ec.principal_title_id
		JOIN eventdetails ed ON ec.id = ed.subcategory_id
		JOIN events e ON ed.event_id = e.event_id
		WHERE pt.title = 'SOINS ET USAGERS';
